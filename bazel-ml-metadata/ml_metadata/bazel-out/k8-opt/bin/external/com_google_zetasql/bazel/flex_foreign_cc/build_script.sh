...
export INSTALLDIR=$EXT_BUILD_ROOT/bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/flex
export BUILD_TMPDIR=$INSTALLDIR.build_tmpdir
export EXT_BUILD_DEPS=$INSTALLDIR.ext_build_deps
export M4="$EXT_BUILD_ROOT/bazel-out/k8-opt-exec-2B5CBBC6/bin/external/com_google_zetasql/bazel/m4/bin/m4"
...
mkdir -p $EXT_BUILD_ROOT/bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/copy_flex/flex
cp -L -r --no-target-directory "$INSTALLDIR" "$EXT_BUILD_ROOT/bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/copy_flex/flex" && find "$EXT_BUILD_ROOT/bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/copy_flex/flex" -type f -exec touch -r "$INSTALLDIR" "{}" \;
cd $EXT_BUILD_ROOT
if [[ -L "bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/flex/bin/flex" ]]; then
  target="$(readlink -f "bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/flex/bin/flex")"
  rm "bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/flex/bin/flex" && cp -a "${target}" "bazel-out/k8-opt/bin/external/com_google_zetasql/bazel/flex/bin/flex"
fi
