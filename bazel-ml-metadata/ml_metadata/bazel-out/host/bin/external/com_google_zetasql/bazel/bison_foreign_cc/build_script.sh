...
echo """"
echo ""Bazel external C/C++ Rules. Building library 'bison'""
echo """"
set -euo pipefail
export EXT_BUILD_ROOT=$(pwd)
export INSTALLDIR=$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/bison
export BUILD_TMPDIR=$INSTALLDIR.build_tmpdir
export EXT_BUILD_DEPS=$INSTALLDIR.ext_build_deps
export M4="$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/m4/bin/m4"
...
symlink_to_dir $EXT_BUILD_ROOT/bazel-out/host/bin/external/rules_foreign_cc/toolchains $EXT_BUILD_DEPS/bin/
children_to_path $EXT_BUILD_DEPS/bin
export PATH="$EXT_BUILD_DEPS/bin:$PATH"
cd $BUILD_TMPDIR
export INSTALL_PREFIX="bison"
...
mkdir -p $EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_bison/bison
cp -L -r --no-target-directory "$INSTALLDIR" "$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_bison/bison" && find "$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_bison/bison" -type f -exec touch -r "$INSTALLDIR" "{}" \;
cd $EXT_BUILD_ROOT
if [[ -L "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/bison" ]]; then
  target="$(readlink -f "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/bison")"
  rm "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/bison" && cp -a "${target}" "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/bison"
fi

if [[ -L "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/yacc" ]]; then
  target="$(readlink -f "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/yacc")"
  rm "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/yacc" && cp -a "${target}" "bazel-out/host/bin/external/com_google_zetasql/bazel/bison/bin/yacc"
fi
