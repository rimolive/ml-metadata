...
echo """"
echo ""Bazel external C/C++ Rules. Building library 'm4'""
echo """"
set -euo pipefail
export EXT_BUILD_ROOT=$(pwd)
export INSTALLDIR=$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/m4
...
mkdir -p $EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_m4/m4
cp -L -r --no-target-directory "$INSTALLDIR" "$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_m4/m4" && find "$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_m4/m4" -type f -exec touch -r "$INSTALLDIR" "{}" \;
cd $EXT_BUILD_ROOT
if [[ -L "bazel-out/host/bin/external/com_google_zetasql/bazel/m4/bin/m4" ]]; then
  target="$(readlink -f "bazel-out/host/bin/external/com_google_zetasql/bazel/m4/bin/m4")"
  rm "bazel-out/host/bin/external/com_google_zetasql/bazel/m4/bin/m4" && cp -a "${target}" "bazel-out/host/bin/external/com_google_zetasql/bazel/m4/bin/m4"
fi
