...
echo """"
echo ""Bazel external C/C++ Rules. Building library 'flex'""
echo """"
set -euo pipefail
export EXT_BUILD_ROOT=$(pwd)
export INSTALLDIR=$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/flex
...
mkdir -p $EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_flex/flex
cp -L -r --no-target-directory "$INSTALLDIR" "$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_flex/flex" && find "$EXT_BUILD_ROOT/bazel-out/host/bin/external/com_google_zetasql/bazel/copy_flex/flex" -type f -exec touch -r "$INSTALLDIR" "{}" \;
cd $EXT_BUILD_ROOT
if [[ -L "bazel-out/host/bin/external/com_google_zetasql/bazel/flex/bin/flex" ]]; then
  target="$(readlink -f "bazel-out/host/bin/external/com_google_zetasql/bazel/flex/bin/flex")"
  rm "bazel-out/host/bin/external/com_google_zetasql/bazel/flex/bin/flex" && cp -a "${target}" "bazel-out/host/bin/external/com_google_zetasql/bazel/flex/bin/flex"
fi
